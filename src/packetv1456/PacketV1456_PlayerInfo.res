...
type difficulty = PacketV1449_PlayerInfo.difficulty
type mode = PacketV1449_PlayerInfo.mode

type t = {
  playerId: int,
  skinVariant: int,
  voiceVariant: int,
  voicePitchOffset: float,
  hair: int,
  name: string,
  hairDye: int,
  hideVisuals: int,
...
module Decode = {
  let {readSingle, readByte, readString, readColor} = module(ErrorAwarePacketReader)
  ...
  let parse = (payload: NodeJs.Buffer.t) => {
    ...
    let? Ok(voiceVariant) = reader->readByte("voiceVariant")
    let? Ok(voicePitchOffset) = reader->readSingle("voicePitchOffset")
    ...
    let mode = if difficultyFlags->BitFlags.flag4 {
      PacketV1449_PlayerInfo.Journey
    } else {
      PacketV1449_PlayerInfo.Classic
    }
...
module Encode = {
  let {packSingle, packByte, packString, packColor, setType, data} = module(ErrorAwarePacketWriter)
  ...
  let toBuffer = (self: t): result<NodeJs.Buffer.t, ErrorAwarePacketWriter.packError> => {
    ...
    ->setType(PacketTypeV1456.PlayerInfo->PacketTypeV1456.toInt)
    ->packByte(self.playerId, "playerId")
    ->packByte(self.skinVariant, "skinVariant")
    ->packByte(self.voiceVariant, "voiceVariant")
    ->packSingle(self.voicePitchOffset, "voicePitchOffset")
