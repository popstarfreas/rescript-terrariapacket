...
type t = {
  ...
  spawnNeedsSyncing: bool,
  shimmerTransparency: bool,
}
...
module Decode = {
  ...
  let readNpcFlags2 = (reader, fieldName) => {
    ...
    Ok({
      ...
      difficulty: flags->BitFlags.flag3,
      spawnNeedsSyncing: flags->BitFlags.flag4,
      shimmerTransparency: flags->BitFlags.flag5,
    })
  }
  ...
  let parse = (payload: NodeJs.Buffer.t) => {
    ...
    let? Ok(difficulty) = if npcFlags2.difficulty {
      let? Ok(difficulty) = reader->readSingle("difficulty")
      Ok(Some(difficulty))
    } else {
      Ok(None)
    }
...
module Encode = {
  ...
  let npcFlags2 = (self: t) => {
    BitFlags.fromFlags(
      ...
      ~flag3=Option.isSome(self.difficulty),
      ~flag4=self.spawnNeedsSyncing,
      ~flag5=self.shimmerTransparency,
      ~flag6=false,
      ~flag7=false,
      ~flag8=false,
    )->BitFlags.toByte
  ...
  let packDifficulty = (writer, difficulty) => {
    switch difficulty {
    | Some(v) => writer->packSingle(v, "difficulty")
    | None => writer
    }
  }
  ...
  let toBuffer = (self: t): result<NodeJs.Buffer.t, ErrorAwarePacketWriter.packError> => {
    ...
    ->setType(PacketTypeV1457.NpcUpdate->PacketTypeV1457.toInt)
    ...
    ->packDifficulty(self.difficulty)
    ->packLife(self.life)
