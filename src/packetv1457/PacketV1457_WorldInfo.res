type eventInfo = {
  ...
  vampireSeed: bool,
  infectedSeed: bool,
  teamBasedSpawnsSeed: bool,
  skyblockWorld: bool,
  dualDungeonsSeed: bool,
}
...
module Decode = {
  ...
  let readEventInfo = reader => {
    ...
    let? Ok(eventInfo8Raw) = reader->readByte("eventInfo8")
    let? Ok(eventInfo9Raw) = reader->readByte("eventInfo9")
    let? Ok(eventInfo10Raw) = reader->readByte("eventInfo10")
    ...
    let eventInfo8 = BitFlags.fromByte(eventInfo8Raw)
    let eventInfo9 = BitFlags.fromByte(eventInfo9Raw)
    let eventInfo10 = BitFlags.fromByte(eventInfo10Raw)
    ...
    let vampireSeed = eventInfo10->BitFlags.flag4
    let infectedSeed = eventInfo10->BitFlags.flag5
    let teamBasedSpawnsSeed = eventInfo10->BitFlags.flag6
    let skyblockWorld = eventInfo10->BitFlags.flag7
    let dualDungeonsSeed = eventInfo10->BitFlags.flag8
  ...
  let parse = (payload: NodeJs.Buffer.t) => {
    ...
    let? Ok(worldUniqueId) = switch Array16.fromArray(worldUniqueIdRaw) {
    | Some(worldUniqueId) => Ok(worldUniqueId)
    | None =>
      Error({
    ...
    let? Ok(extraSpawnPointCount) = reader->readByte("extraSpawnPointCount")
    let extraSpawnPoints: array<Point.t<int>> = []
    let rec readExtraSpawnPoints = idx =>
      if idx >= extraSpawnPointCount {
        Ok()
      } else {
        let? Ok(x) = reader->readInt16(`extraSpawnPointX_${Int.toString(idx)}`)
        let? Ok(y) = reader->readInt16(`extraSpawnPointY_${Int.toString(idx)}`)
        extraSpawnPoints->Array.push({x, y})
...
module Encode = {
  ...
  let packEventInfo = (writer, eventInfo: eventInfo) => {
    ...
    let eventInfo10 = BitFlags.fromFlags(
      ...
      ~flag4=eventInfo.vampireSeed,
      ~flag5=eventInfo.infectedSeed,
      ~flag6=eventInfo.teamBasedSpawnsSeed,
      ~flag7=eventInfo.skyblockWorld,
      ~flag8=eventInfo.dualDungeonsSeed,
    )
    writer
  ...
  let packExtraSpawnPoints = (writer, extraSpawnPoints: array<Point.t<int>>) => {
    let writer = writer->packByte(extraSpawnPoints->Array.length, "extraSpawnPointCount")
    extraSpawnPoints->Array.forEachWithIndex((point, idx) => {
      let idxLabel = Int.toString(idx)
      writer
      ->packInt16(point.x, `extraSpawnPointX_${idxLabel}`)
      ->packInt16(point.y, `extraSpawnPointY_${idxLabel}`)
      ->ignore
    })
    writer
  }
  ...
  let toBuffer = (self: t): result<NodeJs.Buffer.t, ErrorAwarePacketWriter.packError> => {
    ...
    ->ErrorAwarePacketWriter.setType(PacketTypeV1457.WorldInfo->PacketTypeV1457.toInt)
