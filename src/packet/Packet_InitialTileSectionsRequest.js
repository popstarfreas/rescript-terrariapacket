// Generated by ReScript, PLEASE EDIT WITH CARE

import * as PacketType$TerrariaPacket from "../PacketType.js";
import * as ErrorAwarePacketReader$TerrariaPacket from "../ErrorAwarePacketReader.js";
import * as ErrorAwarePacketWriter$TerrariaPacket from "../ErrorAwarePacketWriter.js";
import PacketreaderJs from "@popstarfreas/packetfactory/packetreader.js";
import PacketwriterJs from "@popstarfreas/packetfactory/packetwriter.js";

function parse(payload) {
  let reader = new PacketreaderJs(payload);
  let e = ErrorAwarePacketReader$TerrariaPacket.readInt32(reader, "x");
  if (e.TAG !== "Ok") {
    return e;
  }
  let e$1 = ErrorAwarePacketReader$TerrariaPacket.readInt32(reader, "y");
  if (e$1.TAG === "Ok") {
    return {
      TAG: "Ok",
      _0: {
        x: e._0,
        y: e$1._0
      }
    };
  } else {
    return e$1;
  }
}

function toBuffer(self) {
  return ErrorAwarePacketWriter$TerrariaPacket.data(ErrorAwarePacketWriter$TerrariaPacket.packInt32(ErrorAwarePacketWriter$TerrariaPacket.packInt32(ErrorAwarePacketWriter$TerrariaPacket.setType(new PacketwriterJs(), PacketType$TerrariaPacket.toInt("InitialTileSectionsRequest")), self.x, "x"), self.y, "y"));
}

export {
  parse,
  toBuffer,
}
/* ErrorAwarePacketWriter-TerrariaPacket Not a pure module */
