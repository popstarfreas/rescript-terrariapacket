...
function toBuffer(self) {
  switch (self.TAG | 0) {
    case /* Liquid */0 :
        var liquid = self._0;
        var writer = ManagedPacketWriter$negPacketFactory.setType(new Packetwriter(), PacketType$negTerrariaPacket.toInt(/* NetModuleLoad */77)).packUInt16(/* Liquid */0).packUInt16(liquid.changes.length);
    ...
    case /* ClientText */1 :
        ...
        var message = self._1;
        return ManagedPacketWriter$negPacketFactory.setType(new Packetwriter(), PacketType$negTerrariaPacket.toInt(/* NetModuleLoad */77)).packUInt16(/* Text */1).packString(commandId).packString(message).data;
    case /* ServerText */2 :
        var playerId = self._0;
        var networkText = self._1;
        var color = self._2;
        return ManagedPacketWriter$negPacketFactory.setType(new Packetwriter(), PacketType$negTerrariaPacket.toInt(/* NetModuleLoad */77)).packUInt16(/* Text */1).packByte(playerId).packNetworkText(networkText).packColor(color).data;
    case /* Ping */3 :
        var ping = self._0;
        return ManagedPacketWriter$negPacketFactory.setType(new Packetwriter(), PacketType$negTerrariaPacket.toInt(/* NetModuleLoad */77)).packUInt16(/* Ping */2).packSingle(ping.x).packSingle(ping.y).data;
    case /* Ambience */4 :
        var ambience = self._0;
        return ManagedPacketWriter$negPacketFactory.setType(new Packetwriter(), PacketType$negTerrariaPacket.toInt(/* NetModuleLoad */77)).packUInt16(/* Ambience */3).packByte(ambience.playerId).packInt32(ambience.seed).packByte(ambience.skyEntityType).data;
    case /* Bestiary */5 :
        var bestiary = self._0;
        var writer$1 = ManagedPacketWriter$negPacketFactory.setType(new Packetwriter(), PacketType$negTerrariaPacket.toInt(/* NetModuleLoad */77)).packUInt16(/* Bestiary */4);
