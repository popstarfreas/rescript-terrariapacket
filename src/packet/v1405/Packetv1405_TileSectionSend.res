      ...
      if compressed {
        ...
        if height < 0 || width < 0 {
          Error({
            ...
            error: JsError.make("Invalid dimensions")->JsError.toJsExn,
          })
        } else {
              ...
              if rleCount.contents != 0 {
                ...
                if header4->BitFlags.toByte > 1 {
                  ...
                  if (
                    slopeBits != 0 &&
                      TileSolid.isSolid(
                        tileCache.activeTile->Option.mapOr(0, tile => tile.tileType),
                      )
                  ) {
                    if slopeBits == 1 {
          ...
          | Error(_) =>
            Error({
              ...
              error: JsError.make("Failed to parse entities")->JsError.toJsExn,
            })
          }
        }
      } else {
        Error({
          context: "Packetv1405_TileSectionSend.parse",
          error: JsError.make("Uncompressed TileSectionSend not supported")->JsError.toJsExn,
        })
      }
    } catch {
    | JsExn(obj) => Error({context: "Packetv1405_TileSectionSend.parse", error: obj})
    }
  }
}
...
module Encode = {
  ...
  let toBuffer = (self: t): result<NodeJs.Buffer.t, ErrorAwarePacketWriter.packError> => {
    ...
    | JsExn(obj) => Error({context: "Packetv1405_TileSectionSend.toBuffer", error: obj})
    }
  }
}
...
let toLatest = (self: t): Packet.TileSectionSend.t => {
  height: self.height,
  width: self.width,
  tileX: self.tileX,
  tileY: self.tileY,
  tiles: (self.tiles :> array<array<Packet.TileSectionSend.tile>>),
  chests: (self.chests :> array<Packet.TileSectionSend.Chest.t>),
  signs: (self.signs :> array<Packet.TileSectionSend.Sign.t>),
  entities: (self.entities :> array<Packet.TileSectionSend.Entity.t>),
}

let fromLatest = (latest: Packet.TileSectionSend.t): option<t> => {
  Some({
    compressed: false,
    ...
    tileX: latest.tileX,
    tileY: latest.tileY,
    tiles: (latest.tiles :> array<array<tile>>),
    chests: (latest.chests :> array<Chest.t>),
    signs: (latest.signs :> array<Sign.t>),
    entities: (latest.entities :> array<Entity.t>),
  })
}
